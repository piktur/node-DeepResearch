...
program
  .name("deepresearch")
  .description(
    "AI-powered research assistant that keeps searching until it finds the answer",
  )
  .version(pkg.version)
  .argument("<query>", "The research query to investigate")
  .option(
    "-t, --token-budget <number>",
  ...
  .option(
    "-m, --max-attempts <number>",
    "Maximum bad attempts before giving up",
    (val) => {
      const num = parseInt(val);
