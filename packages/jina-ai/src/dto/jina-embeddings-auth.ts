...
import { AsyncLocalContext } from "#src/lib/async-context.js";
import { JinaEmbeddingsDashboardHTTP } from "#src/lib/billing.js";
import envConfig from "#src/lib/env-config.js";
import logger from "#src/lib/logger.js";
import { InjectProperty } from "#src/lib/registry.js";

import { FirestoreRecord } from "#src/lib/firestore.js";
import { RateLimitDesc } from "#src/rate-limit.js";
...
export class JinaEmbeddingsTokenAccount extends FirestoreRecord {
  ...
  override degradeForFireStore() {
    const copy: any = {
      ...
      wallet: { ...this.wallet },
      // Firebase disability
      customRateLimits: _.mapValues(this.customRateLimits, (v) =>
        v.map((x) => ({ ...x })),
      ),
    };

    for (const field of (this.constructor as typeof JinaEmbeddingsTokenAccount)
      .patchedFields) {
...
export class JinaEmbeddingsAuthDTO extends AutoCastable {
  ...
  async getBrief(ignoreCache?: boolean | string) {
    if (!this.bearerToken) {
      throw new AuthenticationRequiredError({
        message:
          "Jina API key is required to authenticate. Please get one from https://jina.ai",
      });
    }

    let account;
    try {
      account = await JinaEmbeddingsTokenAccount.fromFirestore(
        this.bearerToken,
      );
    } catch (err) {
    ...
    try {
      const r = await this.jinaEmbeddingsDashboard.validateToken(
        this.bearerToken,
      );
      const brief = r.data;
      ...
      await JinaEmbeddingsTokenAccount.save(
        draftAccount.degradeForFireStore(),
        undefined,
        { merge: true },
      );
      ...
      authDtoLogger.warn(`Failed to get user brief: ${err}`, {
  ...
  async reportUsage(
    ...
    return this.jinaEmbeddingsDashboard
        ...
        JinaEmbeddingsTokenAccount.COLLECTION.doc(this.bearerToken!)
          .update({
            "wallet.total_balance":
              JinaEmbeddingsTokenAccount.OPS.increment(-tokenCount),
          })
          .catch((err) => {
            authDtoLogger.warn(`Failed to update cache for ${uid}: ${err}`, {
              err: marshalErrorLike(err),
            });
          });
      ...
      .catch((err) => {
        user.wallet.total_balance += tokenCount;
        authDtoLogger.warn(`Failed to report usage for ${uid}: ${err}`, {
          err: marshalErrorLike(err),
        });
      });
  }
