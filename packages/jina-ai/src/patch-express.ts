...
import { JinaEmbeddingsAuthDTO } from "#src/dto/jina-embeddings-auth.js";
import rateLimitControl, {
  API_CALL_STATUS,
  APICall,
  RateLimitDesc,
} from "#src/rate-limit.js";
import asyncLocalContext from "#src/lib/async-context.js";
import globalLogger from "#src/lib/logger.js";
import { InsufficientBalanceError } from "#src/lib/errors.js";
import { firebaseDefaultBucket, FirestoreRecord } from "#src/lib/firestore.js";
    ...
    try {
      ...
      if (!uid && !ctx.ip) {
        throw new OperationNotAllowedError(
          `Missing IP information for anonymous user`,
        );
      }
      let rateLimitPolicy;
      if (uid) {
      ...
      const criterions = rateLimitPolicy.map((c) =>
        rateLimitControl.rateLimitDescToCriterion(c),
      );
      await Promise.all(
        criterions.map(([pointInTime, n]) =>
          uid
            ? rateLimitControl.assertUidPeriodicLimit(
                uid,
                pointInTime,
                n,
                appName,
              )
            : rateLimitControl.assertIPPeriodicLimit(
                ctx.ip!,
                pointInTime,
                n,
                appName,
      ...
      if (chargeAmount) {
        authDto.reportUsage(chargeAmount, `reader-${appName}`).catch((err) => {
          logger.warn(`Unable to report usage for ${uid || ctx.ip}`, {
            err: marshalErrorLike(err),
          });
        });
      ...
      if (uid && ctx.promptContext.knowledge?.length) {
        Promise.all(
          ctx.promptContext.knowledge.map((x: any) =>
            KnowledgeItem.save(
              KnowledgeItem.from({
                ...x,
                uid,
          ...
          logger.warn(`Failed to save knowledge`, {
            err: marshalErrorLike(err),
          });
        });
      }
      if (ctx.promptContext) {
        ...
        if (Array.isArray(patchedCtx.context)) {
          patchedCtx.context = patchedCtx.context.map((x: object) => ({
            ...x,
            result: undefined,
          }));
        }
        ...
        firebaseDefaultBucket
          ...
          .catch((err: any) => {
            logger.warn(`Failed to save promptContext`, {
              err: marshalErrorLike(err),
            });
          });
      }
      ...
      if (!res.headersSent) {
        ...
        if (err instanceof ApplicationError) {
          res
            .status(parseInt(err.code as string) || 500)
      ...
      logger.error(`Error in billing middleware`, {
        err: marshalErrorLike(err),
      });
      if (err.stack) {
