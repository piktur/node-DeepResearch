...
export const firebaseDefaultBucket = new Storage().bucket(
...
Reflect.set(
  DocumentSnapshot,
  "fromObject",
  patchFireStoreArrogance(Reflect.get(DocumentSnapshot, "fromObject")),
);
...
export type Constructed<T> =
  T extends Partial<infer U> ? U : T extends object ? T : object;
...
export async function fromFirestore(
  ...
  const ref = this.DB.collection(overrideCollection || this.collectionName).doc(
    id,
  );
...
export function fromFirestoreDoc(this: any, snapshot: DocumentSnapshot) {
  const doc = this.from(
    // Fixes non-native firebase types
  ...
  Object.defineProperty(doc, "_ref", {
    value: snapshot.ref,
    enumerable: false,
  });
    ...
    return JSON.parse(
      JSON.stringify(this, function (k, v) {
        ...
        if (
          typeof v === "object" &&
          v &&
          typeof v.degradeForFireStore === "function"
        ) {
